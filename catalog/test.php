...
$beans = R::getAll('SELECT catalog.id, sub.sub_name, catalog.vnutr, catalog.city, unit.unit_name, department.department_name, catalog.cabinet, filial.filial_name, catalog.visibility
    ...
    WHERE unit.id=12 AND department.id=1 AND visibility NOT IN ("0") ORDER BY sub.id');
$output_s .= '
<!DOCTYPE html>
<html>
<head>
...
<body>
  <a href="#" onclick="loadData(1)"><span>Справочник телефонов-></span></a><a href="#" style="color:blue; pointer-events: none">12</a><hr>
  ...
  foreach($beans as $row)
  {
   ...
   <td align="center"><button type="button" name="btn_obr" id="btn_obr" class="btn btn-xs btn-success feedback" data-catalogid="'.$row["id"].'"><span class="glyphicon glyphicon-edit"></span></button></td>
   </tr>
   ';

 }
 $output_s .= '</table></div></body>
</html>';
 echo($output_s);
 $beans = R::getAll('SELECT DISTINCT catalog.unit_id, catalog.department_id, unit.id, unit.unit_name, department.id, department.department_name
  ...
  INNER JOIN unit ON catalog.unit_id = unit.id
  INNER JOIN department ON catalog.department_id = department.id
  WHERE unit.id=12 AND department.id<>1');
 $count=R::count('catalog', ' area_id = ? ', array($areaid->id));
 // print_r($beans);

 // $output.='<div class="col-sm-6" style="background-color: black;"><ul>';
 if ($beans!=null){
  // $output.='<!--h3 style="color:blue">'.$unit_name.fdfdfdf'</h3-->';
  $output .= '<div class="row"><div class="col-sm-6">'.$output_s.'</div>';
  // $output .= '</div></div>';
   $output.='<div class="col-sm-6 well"><ul>';
} else {
  $output .= '<div class="row"><div class="col-sm-12">'.$output_s.'</div></div>';
