...
// var_dump($beans);
  $output_nav .='<div class="sector"><a href="#" onclick="loadData()"><span>Справочник телефонов <span class="glyphicon glyphicon-forward"></span></span></a><a href="#" class="alert alert-info" style="color:blue; pointer-events: none">'.$unit_name.'</a></div><hr>';
  $output_unit .= '
  ...
  foreach($beans as $row)
  {
   ...
   <td align="center"><button class="updown updown'.$row["vnutr"].'" id="upTree'.$row["vnutr"].'" data-action="upTree" data-unitid="'.$unit_id.'" data-unitname="'.$unit_name.'" data-id="'.$row["id"].'" hidden><span class="glyphicon glyphicon-arrow-up"></button></td>
   <td align="center"><button class="updown updown'.$row["vnutr"].'" id="downTree'.$row["vnutr"].'" data-action="downTree" data-unitid="'.$unit_id.'" data-unitname="'.$unit_name.'" data-id="'.$row["id"].'" hidden><span class="glyphicon glyphicon-arrow-down"></span></button></td>
   <td>'.$row["weight"].'</td>
   </tr>';
 }
 if ($beans!=null) {
...
foreach($beans as $row)
{
  if ($row["department_name"]==NULL) {
   ...
   $output .='<li><a onclick="departmentCatalog(this)" style="color:blue; cursor: pointer;" id="'.$row["department_id"].'" data-depname="'.$row["department_name"].'" data-name="'.$row["unit_name"].'" data-unitid="'.$row["unit_id"].'" data-unitname="'.$row["unit_name"].'">'.$row["department_name"].'</li></a>';
 }
}
$output.='</ul></div></div>';
}
